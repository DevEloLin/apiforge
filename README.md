...
- **仅供个人研究、学习与技术评估使用。** 请勿用于生产经营、对外转售或任何商业用途。
- **本项目通过“复用订阅 / CLI 登录凭据”的方式访问各厂商后端，这种做法可能违反
  OpenAI、Anthropic、Google、GitHub、xAI、Cursor、阿里等厂商的服务条款（ToS）。**
  使用它**可能导致你的账号被限流、封禁或注销**。
- 你需**自行承担全部风险**，并自行负责遵守所在地法律与各厂商条款。作者不对任何
  账号损失、数据损失或其他后果负责。
- 本项目**不含、不分发**任何厂商的账号、密钥或订阅；一切凭据由使用者自备（用你
  自己已登录的本机 CLI）。
...
- **Gemini CLI**：`gemini` 登录；设 `GEMINI_OAUTH_ENABLED=1` 启用（实验）。
